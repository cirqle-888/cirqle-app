...
export default function AddExpenseModal({
  ...
  async function recalculateInvoiceTotal(invId: string) {
    ...
    const subtotal = Math.round((taskTotal + expTotal + Number.EPSILON) * 100) / 100
    const total_amount = Math.round((subtotal - discount + tax + prevBal + Number.EPSILON) * 100) / 100
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="bg-card border border-border rounded-2xl shadow-2xl flex flex-col h-full max-h-[90dvh] w-full max-w-xl overflow-hidden">
        {/* Header */}
