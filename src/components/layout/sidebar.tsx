    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-5 border-b border-sidebar-border">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="font-bold text-sidebar-foreground text-sm leading-tight gradient-text">Cirqle</div>
            <div className="text-[10px] text-muted-foreground leading-tight">Design Agency</div>
          </div>
        </div>

        {/* Role badge */}
        <div className="mt-3">
          <span className={`inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full border ${roleBadgeClass[role]}`}>
            {roleLabel[role]}
        ...
        {navSections.map((section, sIdx) => {
          ...
          return (
            <div key={sIdx} className={sIdx > 0 ? 'mt-4' : ''}>
              {section.label && (
                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/40 px-3 mb-1.5">
                  {section.label}
                </p>
              )}
              <div className="space-y-0.5">
                {visibleItems.map(({ label, href, icon: Icon }) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-4 h-4 shrink-0 ${active ? 'text-primary' : 'text-muted-foreground group-hover:text-sidebar-accent-foreground'}`} />
      ...
      <div className="px-3 pt-2 border-t border-sidebar-border">
        ...
        >
          ...
          <div className="flex-1 text-left min-w-0">
            <span className="block leading-tight">{isUnlocked ? 'Privacy unlocked' : 'Privacy locked'}</span>
      ...
      <div className="px-3 py-3">
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-sidebar-accent hover:text-destructive transition-all"
        >
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-60 shrink-0 h-screen flex-col bg-sidebar border-r border-sidebar-border">
        <SidebarContent />
      </aside>
