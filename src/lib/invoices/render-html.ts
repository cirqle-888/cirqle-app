...
export function renderInvoiceHtml(
  ...
  const waIcon = `<img src="data:image/svg+xml,${waIconSvg}" width="17" height="17" style="width:17px;height:17px;display:inline-block;vertical-align:-3px;flex-shrink:0" />`
  const globeIcon = `<img src="data:image/svg+xml,${globeIconSvg}" width="17" height="17" style="width:17px;height:17px;display:inline-block;vertical-align:-3px;flex-shrink:0" />`
...
<body style="padding:${bodyPad};max-width:${(bgStyle === 'shade' || bgStyle === 'custom_images') ? '210mm' : '800px'};margin:0 auto;position:relative;min-height:297mm">
  ...
  <div style="margin-top:auto;padding-top:44px">
    <table style="width:100%;border-collapse:collapse">
      <tr>
        <td style="vertical-align:middle;padding:0;width:42%">${paymentBlock}</td>
        <td style="vertical-align:middle;text-align:center;padding:0 14px;width:16%">${qrBlock}</td>
        <td style="vertical-align:middle;padding:0;width:42%">
