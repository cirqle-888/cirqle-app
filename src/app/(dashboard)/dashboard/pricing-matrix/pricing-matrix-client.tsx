  ...
  return (
    <>
      <Header
        ...
        subtitle={
          // Which-tool-when: this screen sets the shared pool; role rewards
          // are named so the two don't get confused for each other.
          `${allActiveClients.length} client${allActiveClients.length === 1 ? '' : 's'} · ${allActiveServices.length} service${allActiveServices.length === 1 ? '' : 's'} — commission % here sets the shared pool for everyone; a role reward is Settings → Ownership`
        }
        actions={
          <>
