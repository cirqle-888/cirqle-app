 ...
 */
function layerBinding(raw) {
    const compact = normalizeLayerName(raw);
    if (!compact.startsWith('#'))
        return null;
    const alias = TOKEN_ALIASES[compact];
    if (alias)
        return { token: alias, index: null };
    if (CANONICAL_TOKENS[compact])
        return { token: compact, index: null };
    ...
    if (m) {
        ...
        if (CANONICAL_TOKENS[base])
            return { token: base, index: parseInt(m[2], 10) };
    }
...
function readingOrder(nodes) {
    const at = (n) => n.absoluteBoundingBox || { x: n.x, y: n.y };
...
async function applyToken(node, token, product) {
    if (token === IMAGE_TOKEN) {
        if (isText(node))
            return 'skip'; // warned during validation; never fatal here
        if (product.image) {
 ...
 *              them are left.
 */
async function fillCard(card, product, skip) {
    let filled = 0;
    const matched = new Set();
    let imageState = 'none';
    for (const node of descendantsOf(card)) {
        if (skip && skip.has(node.id))
            continue;
        const token = layerToken(node.name);
        if (!token)
            continue;
        ...
        if (result === 'skip')
            continue;
        matched.add(token);
        if (result === 'filled' || result === 'image')
            filled++;
        if (result === 'image')
            imageState = 'placed';
        if (result === 'placeholder' && imageState !== 'placed')
 ...
 */
async function fillLooseLayers(root, products, firstIndex, skip, hideUnused) {
    ...
    for (const node of numbered) {
        ...
        tally(await applyToken(node, layerToken(node.name), product));
    }
    const groups = {};
...
async function fillSelection(products) {
    ...
    for (let i = 0; i < pairs; i++) {
        try {
            ...
            fail('Card ' + (i + 1) + ' (' + cards[i].name + ') could not be filled completely.', 'Fill selected cards', (err instanceof Error ? err.message : String(err)) +
    ...
    if (products.length > cards.length)
        report.overflow = products.length - cards.length;
    figma.ui.postMessage({ type: 'build-done', report });
    figma.notify(`Cirqle Studio: filled ${report.cards} selected card${report.cards === 1 ? '' : 's'}` +
...
async function buildFlyer(msg) {
    ...
    for (let copy = 0; copy < copies; copy++) {
        ...
        try {
            ...
            fail('Could not duplicate the template (copy ' + (copy + 1) + ').', 'Card creation', (err instanceof Error ? err.message : String(err)) + ' — check the template is not locked.');
            continue;
        }
        wrapper.appendChild(card);
        card.x = (copy % cols) * (width + gap);
        card.y = Math.floor(copy / cols) * (height + gap);
        try {
            if (perPage > 1) {
            ...
            fail((perPage > 1 ? 'Page ' : 'Card ') + (copy + 1) + ' (' + card.name + ') could not be filled completely.', 'Layer fill', (err instanceof Error ? err.message : String(err)) +
 ...
 */
const loadedFonts = {};
async function useFont(style) {
    for (const candidate of [style, 'Bold', 'Regular']) {
        if (loadedFonts[candidate])
...
async function label(name, chars, o) {
    ...
    if (o.upper)
        t.textCase = 'UPPER';
    if (o.tracking)
        t.letterSpacing = { value: o.tracking, unit: 'PERCENT' };
    return t;
}
const CARD_W = 220;
const PHOTO_W = 184;
...
async function buildCardComponent(o) {
    ...
    if (o.offer !== 'Price') {
        ...
        ribbon.paddingLeft = 8;
        ribbon.paddingRight = 8;
        ribbon.paddingTop = 5;
        ribbon.paddingBottom = 5;
        ribbon.cornerRadius = 4;
        ribbon.fills = paint(o.offer === 'B1G1' ? COLOR.red : COLOR.navy);
        media.appendChild(ribbon);
        ribbon.appendChild(await label('#badges', o.offer === 'B1G1' ? 'BUY 1 GET 1 FREE' : 'FLAT 50 % OFF', { size: 9, weight: 'Bold', color: COLOR.white, upper: true, tracking: 4 }));
        ribbon.x = -2;
        ribbon.y = 4;
    }
    ...
    const mrp = await label('#mrp', '399', { size: 11, weight: 'Bold', color: COLOR.white, align: 'CENTER', strike: true });
    mrp.opacity = 0.75;
