...
type TemplateInfo = { id: string; name: string; nodeType: string; tokens: string[] }
...
async function fillCard(card: SceneNode, product: BuildProduct): Promise<{ filled: number; missing: string[]; imageState: 'placed' | 'placeholder' | 'none' }> {
  let filled = 0
  ...
  for (const node of descendantsOf(card)) {
    ...
    if (token === IMAGE_TOKEN) {
      if (isText(node)) continue // warned during validation; never fatal here
      if (product.image) {
        ...
        if (imageState !== 'placed') imageState = 'placeholder'
      }
      matched.add(token)
      continue
    }
...
async function buildFlyer(msg: {
  ...
  for (let i = 0; i < msg.products.length; i++) {
    ...
    try {
      ...
      fail(
        'Could not duplicate the template for product ' + (i + 1) + '.',
        'Card creation',
    ...
    ;(card as FrameNode).x = (i % cols) * (width + gap)
    ;(card as FrameNode).y = Math.floor(i / cols) * (height + gap)
    ...
    try {
      ...
      fail(
        'Card ' + (i + 1) + ' (' + card.name + ') could not be filled completely.',
        'Layer fill',
