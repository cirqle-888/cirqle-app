...
contextBridge.exposeInMainWorld('desk', {
  ...
  waMute: (id, muted) => ipcRenderer.send('wa:mute', { id, muted }),
  waPause: (id, paused) => ipcRenderer.send('wa:pause', { id, paused }),
  splitMenu: () => ipcRenderer.send('split:menu'),
  splitAdd: (pane) => ipcRenderer.send('split:add', pane),
  splitClose: (pane) => ipcRenderer.send('split:close', pane),
  webNav: (id, op, url) => ipcRenderer.send('web:nav', { id, op, url }),
  capture: () => ipcRenderer.send('capture:clipboard'),
  retry: (pane) => ipcRenderer.send('retry', pane),
  splitterStart: (i) => ipcRenderer.send('splitter:start', i),
  splitterDrag: (i, screenX) => ipcRenderer.send('splitter:drag', { i, screenX }),
