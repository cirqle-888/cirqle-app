...
|---|---|---|
| `20260904150000_cashbook_expense_markup` | `cashbook_entries.markup_type` + `markup_value` — the rebill cushion chosen when a client expense is recorded, so the auto-copy onto the client's draft invoice applies it instead of billing at cost. | Entries still save: the write learns the missing column from the error and drops it (`retryWithoutMissingColumns`). The Cash Book form hides the cushion section entirely — `columnExists` probes for the column, so the form never takes an answer it cannot keep. The INVOICE-side cushion editor is unaffected and works now: it writes `invoice_expense_items`, whose markup columns have existed since the original feature. |
